<nav id="menu">
    <div id="menu_wrapper">
        <h2>Work</h2>
        <ul class="menu_section menu_section-work">
            <li class="threed seventh"><a href="3d">3D</a></li>
            <li class="artdesign seventh"><a href="artdesign">Art &amp; Design</a></li>
            <li class="flash seventh"><a href="flash">Flash</a></li>
            <li class="music seventh"><a href="music">Music</a></li>
            <li class="photography seventh"><a href="photography">Photography</a></li>
            <li class="programming seventh"><a href="programming">Programming</a></li>
            <li class="web seventh"><a href="web">Web</a></li>
        </ul>
        <h2>Links</h2>
        <ul class="menu_section menu_section-social">
            <li class="facebook seventh"><a href="https://www.facebook.com/jalproductions" target="_blank">Facebook</a></li>
            <li class="deviantart seventh"><a href="http://fu51on.deviantart.com/" target="_blank">DeviantArt</a></li>
            <li class="youtube seventh"><a href="http://www.youtube.com/user/MrLeeProductions" target="_blank">YouTube</a></li>
            <li class="newgrounds seventh"><a href="http://fu5ion.newgrounds.com/" target="_blank">Newgrounds</a></li>
            <li class="github seventh"><a href="https://github.com/jamesl1001" target="_blank">GitHub</a></li>
            <li class="linkedin seventh"><a href="http://www.linkedin.com/profile/view?id=145872637" target="_blank">LinkedIn</a></li>
            <li class="googleplus seventh"><a href="https://plus.google.com/116274038898998688233" target="_blank">Google+</a></li>
        </ul>
        <h2>&nbsp;</h2>
        <p><a href="http://creativecommons.org/licenses/by-nc-nd/3.0/" target="_blank" title="All work by JaL Productions is licensed under a Creative Commons Attribution-NonCommercial-NoDerivs 3.0 Unported License">&copy; JaL Productions 2016</a></p>
    </div>
</nav>